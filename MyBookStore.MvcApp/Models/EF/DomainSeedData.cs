...
public class DomainSeedData
{
    ...
    private List<Review> CreateReviews(List<Book> books, List<User> users)
    {
        ...
        {
            Header = "Книгу не читал, зато играл в сталкер",
            Body = "Вопрос о существовании внеземных цивилизаций волнует человека достаточно давно. Существуют ли" +
                   " инопланетяне или нет, и если существуют, посещают ли они нашу планету и оставляют ли на ней" +
                   " какие-нибудь артефакты? Поэтому для исследования потенциально аномальных зон и были созданы" +
                   " т.н. сталкеры. Являясь сталкером, ты рискуешь вдвойне - сгинуть в неизвестном месте, либо сесть" +
                   " в тюрьму, если выживешь. Но что делать, если это твой единственный способ заработать себе на" +
                   " пропитание? Правильно, крутиться как можешь.Кто-то из друзей или знакомых мне сказал, " +
                   "что \"Пикник на обочине\" это РПГ. Собственно его примерно" +
                   " так и следовало бы экранизировать. Но вот тов. Тарковский решил снять фильм \"Сталкер\" сугубо" +
                   " философским, убрав приключенческую составляющую. Сдаётся мне, молодое поколение вряд ли воспримет" +
                   " фильм (ровно как и книгу) в верном ключе. А то даже бросит на середине и читать/смотреть дальше" +
                   " не станет. Источник: https://www.labirint.ru/books/718360/",
            Rating = 5,
            CreatedBy = users.First(x => x.Name == "User"),
            CreatedAt = DateTime.Now,
            Book = books.First(x => x.Title == "Пикник на обочине")
        });
        
        reviews.Add(new Review
        {
            Header = "Классика советской фантастики.",
            Body = "Просто влюбилась в эту книжную серию! Издание вызывает эстетическое наслаждение: белые странички," +
                   " приятный глазу шрифт, изумительные обложки, а самое главное-крепкий переплет, не" +
                   " вызывающий опасения за сохранность при чтении.\n\n-А что это было? \n-А хрен его знает! Было - и" +
                   " нету, и слава богу.\n\nОднажды группа пришельцев,по пути на громкую вечеринку века, решила" +
                   " сделать привал на планете Земля и утроить пикник. За собой они не прибрали и в последствии на" +
                   " этих местах отдыха образовались аномальные зоны, столь соблазнительные для сталкеров.Хотя кто-то" +
                   " все-таки говорит, что это было настоящее вторжение. Да не суть. Вокруг одной из таких аномальных" +
                   " зон выстроился целый научный городок, где светила науки пытались разобраться в разных \"штучках\"," +
                   " таскаемых из мест отчуждения.Спрос на инопланетные чудеса был велик и за ними предпринимали" +
                   " вылазки и сталкеры, правда они называли это хабром, то есть наживой. У этих смельчаков весьма" +
                   " своеобразный рабочий сленг, особенно на разнообразные аномальные, опасные для жизни места в" +
                   " Зоне. А этого добра там хватает, это и Камаринные плеши, и Ведьмин студень, и Веселые призраки," +
                   " и много чего другого. Но ходит легенда, что в недрах Зоны валяется золотой шар, исполняющий" +
                   " самые сокровенные желания. Правда это или все-таки выдумка? А если и существует он на самом деле," +
                   " то что у него попросить, если посчастливится его найти? Это было мое первое знакомство со" +
                   " Стругацкими и оно оставило после себя крайне положительное впечатление)" +
                   " Источник: https://www.labirint.ru/books/484897/",
            Rating = 5,
            CreatedBy = users.First(x => x.Name == "User"),
            CreatedAt = DateTime.Now,
            Book = books.First(x => x.Title == "Пикник на обочине")
        ...
        reviews.Add(new Review
        {
            Header = "Хорошее издание",
            Body = "Оформление понравилось,текст чистый.Иллюстрации хотелось не яркие." +
            " Источник: https://www.labirint.ru/books/421978/",
            Rating = 4,
            CreatedBy = users.First(x => x.Name == "User"),
            CreatedAt = DateTime.Now,
            Book = books.First(x => x.Title == "Архипелаг Гулаг")
        });
        
        reviews.Add(new Review
        {
            Header = "Легендарная эпопея в одном томе",
            Body = "Мне очень стыдно, но до сего момента этого произведения в моей библиотеке не было совсем. Я" +
                   " брала его читать у кого-то в стародавние времена, когда книгу было не купить.  Почему-то мне" +
                   " всегда хотелось иметь ВСЮ эпопею \"Архипелаг ГУЛАГ\" в ОДНОМ томе. Издательство \"Альфа-книга\"" +
                   " исполнило моё желание. Видимо сделать ТАКОЙ объём с лучшим качеством бумаги и более крупным" +
                   " шрифтом не представляется возможным. Поэтому я придираться особенно не буду, вполне приличное" +
                   " издание за приемлимую цену.  Конечно слегка напрягает маленькое расстояние между строками и" +
                   " местами шрифт сливается, что не очень полезно для глаз, особенно это заметно на страницах," +
                   " где нет диалогов и текст идет \"сплошняком\".  Но, повторюсь, что издатели ухитрились \"впихнуть\"" +
                   " ОГРОМНЫЙ объём в ОДИН том, даже добавить небольшое количество фотографий, и при этом качество не" +
                   " слишком пострадало. Так что о покупке не жалею.  Книга ОЧЕНЬ толстая, хорошо раскрывается," +
                   " бумага белая, чуть просвечивает, шрифт не слишком мелкий, вполне читаемый, но расстояние между" +
                   " строками маленькое, блок прошит. Тираж 6000 экземпляров." +
                   " Подробнее: https://www.labirint.ru/books/208366/",
            Rating = 5,
            CreatedBy = users.First(x => x.Name == "User"),
            CreatedAt = DateTime.Now,
            Book = books.First(x => x.Title == "Архипелаг Гулаг")
